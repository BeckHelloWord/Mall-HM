...
import {Toast } from 'mint-ui';
	export default {
					...
					success: (result) => {	   
                        if(result.status=="201"){
                            ...
                            if(result.data.ordStatus=="notFinace"||result.data.ordStatus=="finace"){
                                ...
                                this.endTime = endDate.getFullYear()+"-"+(endDate.getMonth()+1)+"-"+endDate.getDate()+" "+endDate.toString().split(" ")[4]
                            }else{
		...
		mounted: function() {
            ...
            this.startTime = nowDate.getFullYear()+"-"+(nowDate.getMonth()+1)+"-"+nowDate.getDate()+" "+nowDate.toString().split(" ")[4]
			window.Hub.$emit('refresh', null);
			window.Hub.$emit('setTitle', "收银台", false, null," ", null)
			window.Hub.$emit('setFooter', false);
            if (config.isWeiXin()) {
               $(".paySuccess").height(localStorage.getItem("webViewHeight"));//按钮
            }else{
                $(".paySuccess").height(localStorage.getItem("webViewHeight")-50);//按钮
            }
