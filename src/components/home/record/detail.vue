...
<template>
	<div class="recordDetail">
		<div class="content">
            ...
            <router-link <router-link :to="{ path: '/invest/detail/'+detail.investId+'?type='+detail.type}" tag='p'>
                {{detail.borrowTitle}}
                <span class="transfer" v-show='turnOff'>转</span>
            </router-link>
			<p>投资时间&nbsp;&nbsp;{{detail.dateCreated}}</p>
			<ul class="clearfix">
				<li><p>投资金额(元)</p><p>{{detail.amount | formatMoney}}</p></li>
				<li>
					<p v-show='!(turnOff&& (detail.shouldInterest==0 || detail.alreadyInterest ==0))' @click='help'>{{bool?'预期收益(元)':'实收收益(元)'}}</p>
					<p v-show='!(turnOff&& (detail.shouldInterest==0 || detail.alreadyInterest ==0))'>{{detail.shouldInterest | formatMoney}}</p>
					<img v-show='turnOff&& (detail.shouldInterest==0 || detail.alreadyInterest ==0)' src="../../../assets/images/transferred.png" height="48" alt="已转让">
				</li>
			</ul>
		</div>
			...
			initData(){
				this.$public.API_GET({
					...
					success : (result) => {
                        ...
                        this.turnOff = this.detail.type == 'bond'
					}
				});
			}
		},
