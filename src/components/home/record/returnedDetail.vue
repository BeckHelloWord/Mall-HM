	...
	<div class="returnedDetail">
		<ul>
			<li v-for='(item,k) in receiptWaitList'>
				...
				<div>
					<p class="clearfix">
						<span>{{item.period}}期</span><span>单位：元</span>
					</p>
				</div>
				<div>
					<div @click='open(k)'>
						...
						<p>
							<span>{{status?"待回":"已回"}}</span>
							<span :class="{'more':bool[k]}">利息 +{{item.alreadyInterest ? item.alreadyInterest : item.shouldInterest | formatMoney}}</span>
						</p>
						<p v-show='bool[k]' class="clearfix">
							<span></span>
							<span :class="{'more':bool[k]}">本金 +{{item.alreadyCapital ? item.alreadyCapital : item.shouldCapital | formatMoney}}</span>
						</p>
					</div>
				</div>
			</li>
...
<script>
	...
	export default{
		...
		methods:{
			...
			toFalse:function(){
				this.bool=[]
                this.receiptWaitList.forEach((v,i)=>{
            ...
            p {
                ...
                span:last-child {
                    ...
                    i {
                        ...
                        width:  pxToRem(  0*2);
                        height:  pxToRem(  0*2);
                        border-top: pxToRem(  5*2) solid #999;
                        border-bottom: pxToRem(  5*2) solid transparent;
                        ...
                        &.more {
                            ...
                            -webkit-transform: rotateZ(180deg) translateY(-4px);
                               -moz-transform: rotateZ(180deg) translateY(-4px);
                                -ms-transform: rotateZ(180deg) translateY(-4px);
                                 -o-transform: rotateZ(180deg) translateY(-4px);
                            border-top: pxToRem(  5*2) solid $red;
                        }
                    }
                }
                &:nth-child(n+2) {
