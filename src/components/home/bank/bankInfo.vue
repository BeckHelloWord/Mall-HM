  ...
  <div class="recharge-all">
    <div class="page-popup">
      <form id="bankInfoForm" class="bankInfoForm">
          <mt-cell title="开户省份" is-link  @click.native="popupVisible4 = true" size="large" class="selCity"  :class="bankProvinceTest">
            <span class="bankProvince" >{{ addressProvince }} {{ addressCity }}</span>
          </mt-cell>
        <mt-field label="开户支行" placeholder="请填写开户支行" v-model="bankSubName" :class="bankSubNameTest"></mt-field>
        <pre class="bankInfomsg">{{getMemberBankPhone.bankMessage}}</pre>
        <div :class="btnClass" @click="bankInfoBtn">保存</div>
      </form>
  ...
  export default {
    ...
    mounted() {
      ...
      window.Hub.$emit('setFooter',false);
		  ...
		  //从银行卡详情带入的参数直接显示,带入的是空文字是灰色
      var addBankProvince = sessionStorage.getItem("addBankProvince");
      var addBankCity = sessionStorage.getItem("addBankCity");
      var addBankSubName = sessionStorage.getItem("addBankSubName");

      if(addBankProvince || addBankCity){
        this.addressProvince = addBankProvince;
        this.addressCity = addBankCity;
      }
      //从银行卡详情带入的参数直接显示,带入的是空文字是灰色
      if(addBankSubName != "请填写开户支行"){
        this.bankSubName =addBankSubName;
      }
