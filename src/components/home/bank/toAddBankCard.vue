...
<template>
  <div class="recharge-all">
    <div id="addBankCard" class="addBankCard">
      <div class="page-part">
        <div v-if="readonlyName">
          <mt-field label="持卡人" placeholder="请输入持卡人姓名" v-model="realName" style="display:none;"></mt-field>
          <a class="mint-cell mint-field noBd">
            <div class="mint-cell-left"></div>
            <div class="mint-cell-wrapper">
              <div class="mint-cell-title"><span class="mint-cell-text">持卡人</span></div>
              <div class="mint-cell-value">{{realName | formatName}}</div>
            </div>
          </a>
        </div>
        <div v-else class="noBd3">
          <mt-field label="持卡人" placeholder="请输入持卡人姓名" v-model="realName" v-validate="'required|name'" data-vv-value-path="innerValue" data-vv-as=" " data-vv-name="nameTips" :has-error="errors.has('nameTips')"></mt-field>
          <span v-show="errors.has('nameTips')" class="help is-danger">{{ errors.first('nameTips') }}</span>
        </div>
        <div v-if="readonlyIdCard">
          <mt-field label="身份证" placeholder="请输入持卡人身份证" v-model="idCard" style="display:none;"></mt-field>
          <a class="mint-cell mint-field">
            <div class="mint-cell-left"></div>
            <div class="mint-cell-wrapper">
              <div class="mint-cell-title"><span class="mint-cell-text">身份证</span></div>
              <div class="mint-cell-value">{{idCard | formatBankCard}}</div>
            </div>
          </a>
        </div>
        <div v-else class="noBd2">
          <mt-field label="身份证" placeholder="请输入持卡人身份证" v-model="idCard" v-validate="'required|idCard'" data-vv-value-path="innerValue" data-vv-as=" " data-vv-name="idCardTips" :has-error="errors.has('idCardTips')"></mt-field>
      ...
      <div class="page-part">
        ...
        <mt-field label="银行卡卡号" placeholder="请输入银行卡卡号" v-model="bankAccount" type="text" :attr="{maxlength:19}"  :readonly="readonlyBankAccount" v-validate="'required|bank'" data-vv-value-path="innerValue" data-vv-as=" " data-vv-name="bankNoTips" :has-error="errors.has('bankNoTips')"></mt-field>
      ...
      <div :class="btnClass" @click="sure">确认</div>
    </div>
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      ...
      bankNoFun(){
        var value =  /^[0-9]{16,19}$/.test(this.bankAccount);
      ...
      cardBin(){
        var verifyCard = {flag: "mobile", bankAccount: this.bankAccount};
        this.$public.API_GET({
          url: 'getBankName',
          data: verifyCard,
          success: (result) => {
    ...
    mounted: function() {
      ...
      if(myInfo &&  myInfo.memberMobile){
         this.mobile = myInfo.memberMobile;
      }
      if(myInfo &&  myInfo.realName){
        this.realName = myInfo.realName;
      }
      if(myInfo &&  myInfo.idCard){
        this.idCard = myInfo.idCard;
      }
      if(myInfo &&  myInfo.bankName){
        this.bankName = myInfo.bankName;
      }
      if(myInfo &&  myInfo.bankAccount){
        this.bankAccount = myInfo.bankAccount;
      }     
      ...
      if (config.isWeiXin()) {
           $(".recharge-all").height($(window).height());//按钮
      }else{
...
<style lang="scss" scoped="" type="text/css">
  ...
  .recharge-all{
    .addBankCard{
      ...
      .module-list-p {padding: 10px;font-size: pxToRem(24);color:$ft-999;}
      .addBankCardBtn{@include btn;}
    }
  .mint-popup{width:100%;}
  .bankProvince{color:$black;}
  .red{@include btn;}
