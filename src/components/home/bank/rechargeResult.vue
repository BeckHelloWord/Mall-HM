  ...
  <div class="recharge-all">
   <div class="rechargeResult">
     <div class="rechargeSuccess" v-if="resultSuccess">
       ...
       <h3>恭喜您，充值成功</h3>
       <h6>充值：{{amount | formatMoney}}元</h6>
       <span>{{time}}</span>
       <router-link to="/home/index" class="rechargeSuccessBtn">确定</router-link>
     </div>
     <div class="rechargeFail" v-else>
       <div class="img"><img src="../../../assets/images/rechargeFail.png"></div>
       <h3>很遗憾，充值失败</h3>
       <ul>
         <li class="clearfix"><div class="li"><h6>充值：{{amount | formatMoney}}元</h6><span>{{time}}</span></div></li>
         <li class="clearfix"><div class="li"><p>请重试，或联系想念客服</p></div></li>
       </ul>
       <router-link to="/home/index" class="rechargeFailBtn">确定</router-link>
     </div>
   </div>
  </div>
</template>
...
<script type="text/javascript">
  export default {
    ...
    mounted: function() {
      ...
      //充值带的时间
        if(window.Hub.recharge){
          this.time = window.Hub.recharge.time;
        }
...
.rechargeResult{
  ...
  span{font-size: pxToRem(20);color:$ft-999;}
  ...
  p{color:$ft-666;}
  .rechargeSuccessBtn{@include btn-bottom;}
