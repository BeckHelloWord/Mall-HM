<template>
	<div class="assets">
		<div>
			...
			<div v-show='isTotalAsset' id="map" style="height:225px;">
				<IEcharts :option="pie"></IEcharts>
			</div>
			<!-- <div v-show='totalAsset==0' class="totalAssetZoom">
					<img src="../../assets/images/user-center.png" height="225">
					<span class="earnings">4.2%~15%</span>
			</div> -->
			<router-link to="/invest" tag="div" v-show='!isTotalAsset' class="totalAssetZoom">
				<img src="../../assets/images/user-center.png" height="225">
				<span class="earnings">4.2%~15%</span>
			</router-link>
			<div class="content">
	...
	export default{
		...
		methods:{
			...
			initMap(){
				...
				var d_length = this.pie.series[0].data.length

				for(var i = d_length-1;i>=0;i--){
					if(this.pie.series[0].data[i].value==0){
						// console.log(i)
						this.pie.color.splice(i,1)
						this.pie.series[0].data.splice(i,1)
					}
				}
					// console.log(this.pie.series[0].data)

				if(this.pie.color.length==0){
					this.pie.color.push('#c3c3c3')
					this.pie.series[0].data.push(0)
				}
			...
			initData(){
				this.$public.API_GET({
	                ...
	                success : (result)=>{
						...
						this.isTotalAsset = this.totalAsset!=0;
						// 可用余额
